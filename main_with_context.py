...
from mongo_vector import get_related_context

load_dotenv()

def send_prompt(prompt, system_prompt, related_context=None, model="gpt-4o-mini"):
    ...
    # Add related context to the system prompt if available
    if related_context:
...
"""
    ...
    for message in user_messages:
        print("\n=========================\n")
        print("User:", message)
        related_context = get_related_context(message, top_k=3)
        if related_context:
            print("Related context found:", related_context)
        else:
            print("No related context found.")
        send_prompt(message, system_prompt, related_context)
